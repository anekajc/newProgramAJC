        ...
        <table id="tabel" class="table table-sm table-bordered table-striped" style="font-size:13px; width:100%;">
          <thead class="text-center bg-primary text-white">
            <tr>
              ...
              <th style="white-space: nowrap;">Gudang Asal</th>
              <th style="white-space: nowrap;">Gudang Tujuan</th>
              <th style="white-space: nowrap;">Sat</th>
              <th style="white-space: nowrap;">Qnt Sisa</th>
            </tr>
          </thead>
          <tbody id="tabel_data" class="text-left" >
            ...
            <tr data-nobukti="{{ $tempOutstanding[$i]->NOBUKTI }}" data-urut="{{ $tempOutstanding[$i]->URUT }}">
              ...
              <td>{{ $tempOutstanding[$i]->GdgAsal}}</td>
              <td>{{ $tempOutstanding[$i]->GdgTujuan}}</td>
              <td>{{ $tempOutstanding[$i]->Satx}}</td>
              <td>{{ $tempOutstanding[$i]->Qntx}}</td>
            </tr>
              @endfor
            </tbody>
        </table>
        ...
        <table id="tabel2" class="table table-sm table-bordered table-striped" style="font-size:13px; width:100%;">
          <thead class="text-center bg-primary text-white">
            <tr>
              ...
              <th style="white-space: nowrap;">Qnt</th>
              <th style="white-space: nowrap;">Qnt Batal</th>
          ...
          <tbody id="tabel2_data" class="text-left" >
            ...
            <tr data-nobukti="{{ $tempPenerimaan[$i]->NOBUKTI }}" data-urut="{{ $tempPenerimaan[$i]->URUT }}">
              ...
              <td>{{ $tempPenerimaan[$i]->GdgAsal}}</td>
              <td>{{ $tempPenerimaan[$i]->GdgTujuan}}</td>
              <td>{{ $tempPenerimaan[$i]->Satx}}</td>
              <td>{{ $tempPenerimaan[$i]->QNT}}</td>
              <td>{{ $tempPenerimaan[$i]->Qntx}}</td>
            ...
            res2.tempPenerimaan.forEach((item) => {
              rowTable2 += `
                <tr data-nobukti="${item.NOBUKTI}" data-urut="${item.URUT}">
                  ...
                  <td class="text-right">${formatAngka(item.QntBatal)}</td>
    ...
    success: function (res) {
      ...
      res.tempOutstanding.forEach((item) => {
        rowTable += `
          <tr data-nobukti="${item.NOBUKTI}" data-urut="${item.URUT}">
            <td class="text-center" style="width:100px;">
              <button class="btn btn-primary btn-sm" title="Kunci Per Barang" 
                onclick="lockItem('${item.NOBUKTI}', '${item.URUT}', '${item.KODEBRG}')">
                <i class="bi bi-lock-fill"></i>
              </button>
              <button class="btn btn-success btn-sm" title="Kunci Per No Bukti" 
                onclick="lockAll('${item.NOBUKTI}')">
                <i class="bi bi-lock-fill"></i>
              </button>
            </td>
            <td>${item.NOBUKTI}</td>
            ...
            <td>${item.GdgTujuan}</td>
            <td>${item.Satx}</td>
            <td class="text-right">${formatAngka(item.Qntx)}</td>
          </tr>`;
      });
      ...
      res.tempPenerimaan.forEach((item) => {
        rowTable2 += `
          <tr data-nobukti="${item.NOBUKTI}" data-urut="${item.URUT}">
            <td class="text-center">
              <button class="btn btn-warning btn-sm" title="Buka Per Barang" 
                onclick="submitUnlock('${item.NOBUKTI}', 'item', '${item.URUT}', '${item.KODEBRG}')">
                <i class="bi bi-unlock"></i>
              </button>
              <button class="btn btn-danger btn-sm" title="Buka Per No Bukti" 
                onclick="submitUnlock('${item.NOBUKTI}', 'all')">
                <i class="bi bi-unlock-fill"></i>
              </button>
            </td>
            <td>${item.NOBUKTI}</td>
