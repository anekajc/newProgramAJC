...
<div id='page1' class="container-fluid">
  ...
  <div class="row">
    ...
    <div class="col-6 text-right">
      ...
      onclick="buttonAdd()">Add Penyerahan</button>
    </div>
...
<div id="page2" class="container-fluid" style="display: none; margin-top:-80px;" >
  <div class="row">
    <div class="col-6 text-left">
      <h1>Form Retur Penyerahan Sample</h1>
    </div>
    <div class="col-6 text-right">
...
<div id="page3" class="container-fluid" style="display: none; margin-top:-80px;">
        <div class="row">
          <div class="col-6 text-left">
            <h2>Detail Retur Penyerahan Sample</h2>
          </div>
          <div class="col-6 text-right">
            <button type="button" class="btn btn-danger btn-lg" style="
            height: 30px; 
...
function loadAll () {
  ...
  $.ajax({
    url: "{!! url('returpenyerahansampleloadall') !!}",
    type: "GET",
    data: { _token },
...
function submitPrint (nobukti) {
    ...
    $.ajax({
      url: "{!! url('returpenyerahansampledetailCetak') !!}",
      type: "post",
      async: false,
      data: {
        _token : _token,
...
function buttonOtorisasi (nobukti, isOtorisasi) {
  ...
  $.ajax({
    url: "{!! url('returpenyerahansampleupdateotorisasi') !!}",
    type: "post",
    async: false,
    data: {
      _token,
  ...
  function(evt, value) {
    ...
    let xpket = value;
     ...
     if (xpket==''){
      ...
      $.ajax({
        url: "{!! url('returpenyerahansampleupdatebatalotorisasi') !!}",
        type: "post",
        async: false,
        data: {
          _token,
...
function buttonAddListSerahSample () {
  ...
  $('#tabel_add_list_serahsample').DataTable().destroy();

  // exclude 
  let excludeToSend = excludeSerahSamples.filter(e => !(e.nobukti === nobukti && e.urut === urut));

  $.ajax({
    url: "{{ url('returpenyerahansamplenosample') }}",
    type: "get",
    data: {
      sales: sales,
      nobukti: nobukti,
...
function buttonAddListSales () {
  ...
  $.ajax({
    url: "{{ url('returpenyerahansamplelistsales') }}",
    type: "get",
    async: false,
    success: function(res) {
      console.log(res);
...
function submitAddEdit () {
  ...
  $.ajax({
    url: "{!! url('returpenyerahansamplespadd') !!}",
    type: "POST",
    async: false,
    data: {
      _token,
...
function buttonEdit (nobukti, isOtorisasi1) {
  ...
  $.ajax({
    url: "{!! url('returpenyerahansamplespdetail') !!}",
    type: "get",
    async: false,
    data: { nobukti },
    success: function (res) {
...
function buttonAddDeleteItem (index) {
  ...
  alertify.confirm('Hapus Item', `Apakah yakin ingin menghapus item ${data.KODEBRG}?`, function () {
    ...
    $.ajax({
      url: "{!! url('returpenyerahansamplespdelete') !!}",
      type: "post",
      async: false,
      data: {
        _token,
...
function setNewNoBukti () {
  $.ajax({
    url: "{!! url('returpenyerahansamplespnobukti') !!}",
    type: "get",
    async: false,
    data: {
    },
...
function searchBarangAll (e) {
  ...
  //         console.log(err);
  //         alertify.warning('Terjadi kesalahan, silakan refresh browser');
  //         $("#tabel_data_add_list_item").html(`<tr><td class="text-center" colspan="5">Gagal mengambil data</td></tr>`);
  //       }
  //     });
  //   }
  // }
