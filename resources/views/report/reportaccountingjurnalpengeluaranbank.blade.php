...
{{-- Table styling lives in public/css/report-table.css (loaded via report/newmaster2.blade.php).
     Jurnal Pengeluaran Bank (BBK): styled .tb-report dengan header dua tingkat (Debet → Perk./Jumlah,
     ...
     pada baris membuka voucher Bukti Bank (CetakKasharian) di panel bawah (report-table.js). --}}
<style>
...
<div class="tb-report main">
  <div class="content">
    ...
    <div class="toolbar">
      <div>
        <div class="page-title">Jurnal Pengeluaran Bank</div>
...
<script type="text/javascript">
  ...
  const reportUrl = "{{ url('laporanaccountingjurnalpengeluaranbank_doReport') }}";

  // Bottom voucher panel endpoints (report-table.js dimuat oleh masterreport2).
  // No. Bukti di jurnal ini adalah BBK (Bukti Bank Keluar) → openVoucher memakai
  // jenisFromNo() yang memetakan BBK ke CetakKasharian (kasUrl).
  window.ReportTableConfig = {
    kasUrl    : "{{ url('laporanaccountingjurnalpengeluaranbank_doKasharian') }}",
    invoiceUrl: "{{ url('laporanaccountingjurnalpengeluaranbank_doInvoice') }}",
    lpbUrl    : "{{ url('laporanaccountingjurnalpengeluaranbank_doLpb') }}",
    bpUrl     : "{{ url('laporanaccountingjurnalpengeluaranbank_doBp') }}"
  };
  ...
  function exportDelimited(fmt) {
    ...
    a.download = 'JurnalPengeluaranBank_' + (globalDate1 || '') + '_' + (globalDate2 || '') + '.' + ext;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    showToast('📄', 'Data diekspor sebagai ' + fmt);
  }

  /* ── LOAD DATA (Sp_LapJurnal 'BBK', divisi, date1, date2; array biasa) ── */
  function makeTable(_mode) {
    globalDate1 = $('#inputDate1').val();
    globalDate2 = $('#inputDate2').val();
    g_reportTitle = 'REPORT JURNAL PENGELUARAN BANK';
  ...
  // "Saldo Awal"). Klik dilakukan pada SELURUH baris (lihat voucherRowOpen); Jenis (BBK)
  ...
  // bisa diklik untuk membuka voucher Bukti Bank (report-table.js).
