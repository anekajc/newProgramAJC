...
{{-- Table styling lives in public/css/report-table.css (loaded via report/newmaster2.blade.php).
     Jurnal Pengeluaran Kas (BKK): styled .tb-report dengan header dua tingkat (Debet → Perk./Jumlah,
     Kredit → Perk./Jumlah). Baris dikelompokkan per No. Bukti dengan Subtotal per voucher + Grand
     Total. Filter: Periode (rentang tanggal) + Divisi (DROPDOWN, sumber loadDivisi). Klik No. Bukti
     membuka voucher Bukti Kas (CetakKasharian) di panel bawah (report-table.js). --}}
<style>
...
<div class="tb-report main">
  <div class="content">
    ...
    <div class="toolbar">
      <div>
        <div class="page-title">Jurnal Pengeluaran Kas</div>
...
<script type="text/javascript">
  ...
  const reportUrl = "{{ url('laporanaccountingjurnalpengeluarankas_doReport') }}";

  // Bottom voucher panel endpoints (report-table.js dimuat oleh masterreport2).
  // No. Bukti di jurnal ini adalah BKK (Bukti Kas Keluar) → openVoucher memakai
  // jenisFromNo() yang memetakan BKK ke CetakKasharian (kasUrl).
  window.ReportTableConfig = {
    kasUrl    : "{{ url('laporanaccountingjurnalpengeluarankas_doKasharian') }}",
    invoiceUrl: "{{ url('laporanaccountingjurnalpengeluarankas_doInvoice') }}",
    lpbUrl    : "{{ url('laporanaccountingjurnalpengeluarankas_doLpb') }}",
    bpUrl     : "{{ url('laporanaccountingjurnalpengeluarankas_doBp') }}"
  };
  ...
  function exportDelimited(fmt) {
    ...
    a.download = 'JurnalPengeluaranKas_' + (globalDate1 || '') + '_' + (globalDate2 || '') + '.' + ext;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    showToast('📄', 'Data diekspor sebagai ' + fmt);
  }

  /* ── LOAD DATA (Sp_LapJurnal 'BKK', divisi, date1, date2; array biasa) ── */
  function makeTable(_mode) {
    globalDate1 = $('#inputDate1').val();
    globalDate2 = $('#inputDate2').val();
    g_reportTitle = 'REPORT JURNAL PENGELUARAN KAS';
  ...
  // baris "Saldo Awal"). Membuka panel voucher bawah (report-table.js); Jenis (BKK)
