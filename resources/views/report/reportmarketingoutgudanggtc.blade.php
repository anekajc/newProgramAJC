...
<div class="tb-report main">
  <div class="content">
    ...
    <div class="toolbar">
      {{-- <div>
        <div class="page-title">Report Marketing - Outstanding Gudang GTC</div>
      </div> --}}
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="rt-section">
          ...
          {{-- Tidak ada switcher "Tampilan": mode Rekap yang dulu ada di halaman ini tidak
               pernah bisa diakses user (toggle-nya `hidden` di versi lama) dan kolomnya adalah
               copy-paste dari laporan lain yang tidak sesuai data aging/stok di sini -- dihapus,
               bukan dipindah. Order By juga tidak ada: controller selalu memanggil
               Sp_ReportOutSpbDet dengan Ordr = "Y" (nilai tetap, tidak ada dropdown untuk ini
               di versi lama halaman ini). --}}
          <div class="rt-grid-1">
            <div>
              <label class="rt-field-label" for="modalOtorisasi">Otorisasi</label>
              {{-- Param ke-6 Sp_ReportOutSpbDet (dulu di-hardcode 1, tidak ada kontrolnya sama
                   sekali di halaman ini): posisinya sama seperti NeedOto di keluarga proc SPB
                   lain, jadi dipakai enum yang sama -- 0 = Sudah Otorisasi, 1 = Belum Otorisasi,
                   2 = Semua. Sama seperti reportmarketingoutsppb.blade.php /
                   reportmarketingoutspbhrgso.blade.php (proc yang sama), belum diverifikasi
                   langsung ke definisi SP. --}}
...
<script type="text/javascript">
  ...
  const reportUrl = "{{ url('laporanmarketingoutgudanggtc_doReport') }}";

  // Satu-satunya mode: Detail. Rekap dihapus (lihat komentar di modal Filter) -- tidak ada
  // switcher "Tampilan" untuk halaman ini.
  g_modeReport = 0;
  ...
  function exportDelimited(fmt) {
    ...
    a.download = 'OutstandingGudangGTC_' + (globalDate1 || '') + '_' + (globalDate2 || '') + '.' + ext;
  ...
  // Kolom ini TIDAK diverifikasi terhadap definisi SP (tidak ada source SP di repo) --
  // controller memanggil Sp_ReportOutSpbDet dengan Ordr = "Y", nilai yang tidak dipakai
  // halaman lain manapun di repo ini (dua sibling yang memanggil proc yang sama memakai
  // Ordr = "X" dan punya kolom yang sama sekali berbeda). Dipertahankan apa adanya karena
  // bentuknya spesifik untuk laporan aging/stok (SAT_1, Umur), bukan boilerplate copy-paste
  // generik seperti kolom Rekap yang dihapus dari halaman ini.
