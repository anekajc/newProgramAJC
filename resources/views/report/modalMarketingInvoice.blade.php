...
function buttonPilih(idPart, selectedData)
  {
      ...
      {
        $("#inputJenis").val(selectedData);
        $("#formSelect").modal("hide");
      }
      else if (idPart == 8)
      {
        $("#inputGroup2").val(selectedData);
        $("#formSelect").modal("hide");
      }
...
function buttonSelect (idModal)
{
  ...
  else if (idModal == "selectJenis")
  {
    loadSelectJenis();
  }
  else if (idModal == "selectGroup2")
  {
    loadSelectGroup2();
  }
...
function loadSelectJenis() {
  let _token = $("#_token").val();

  $('#tabelSelect').DataTable().destroy();

  $.ajax({
    url: "{!! url('laporanmarketinginvoice_loadjenis') !!}",
    type: "get",
    async: false,
    data: {
      _token: _token,
  ...
  let rowTable = "";
  dataRefresh.forEach((item, i) => {
    rowTable += pickerRowHtml('7', item.KodeJenis,
      `<td>${item.KodeJenis}</td><td>${item.NamaJenis}</td>`);
  });
...
function loadSelectGroup2() {
  let _token = $("#_token").val();

  $('#tabelSelect').DataTable().destroy();

  $.ajax({
    url: "{!! url('laporanmarketinginvoice_loadgroup') !!}",
    type: "get",
    async: false,
    data: {
      _token: _token,
  ...
  document.getElementById("tabelHeader").innerHTML = pickerHeadHtml(['Kode Group Cust', 'Nama']);

  let rowTable = "";
  dataRefresh.forEach((item, i) => {
    rowTable += pickerRowHtml('8', item.KODEGROUPCUSTSUPP,
      `<td>${item.KODEGROUPCUSTSUPP}</td><td>${item.NamaGROUPCUSTSUPP}</td>`);
  });
