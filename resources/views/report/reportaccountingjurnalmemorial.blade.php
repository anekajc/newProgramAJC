...
{{-- Table styling lives in public/css/report-table.css (loaded via report/newmaster2.blade.php).
     Jurnal Memorial (BMM): styled .tb-report dengan header dua tingkat (Debet → Perk./Jumlah,
     ...
     pada baris membuka voucher Memorial (CetakKasharian) di panel bawah (report-table.js). --}}
<style>
...
<div class="tb-report main">
  <div class="content">
    ...
    <div class="toolbar">
      <div>
        <div class="page-title">Jurnal Memorial</div>
...
<script type="text/javascript">
  ...
  const reportUrl = "{{ url('laporanaccountingjurnalmemorial_doReport') }}";

  // Bottom voucher panel endpoints (report-table.js dimuat oleh masterreport2).
  // No. Bukti di jurnal ini adalah BMM (Memorial) → openVoucher memakai jenisFromNo()
  // yang memetakan BMM ke CetakKasharian (kasUrl).
  window.ReportTableConfig = {
    kasUrl    : "{{ url('laporanaccountingjurnalmemorial_doKasharian') }}",
    invoiceUrl: "{{ url('laporanaccountingjurnalmemorial_doInvoice') }}",
    lpbUrl    : "{{ url('laporanaccountingjurnalmemorial_doLpb') }}",
    bpUrl     : "{{ url('laporanaccountingjurnalmemorial_doBp') }}"
  };
  ...
  function exportDelimited(fmt) {
    ...
    a.download = 'JurnalMemorial_' + (globalDate1 || '') + '_' + (globalDate2 || '') + '.' + ext;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    showToast('📄', 'Data diekspor sebagai ' + fmt);
  }

  /* ── LOAD DATA (Sp_LapJurnal 'BMM', divisi, date1, date2; array biasa) ── */
  function makeTable(_mode) {
    globalDate1 = $('#inputDate1').val();
    globalDate2 = $('#inputDate2').val();
    g_reportTitle = 'REPORT JURNAL MEMORIAL';
  ...
  // "Saldo Awal"). Klik dilakukan pada SELURUH baris (lihat voucherRowOpen); Jenis (BMM)
  ...
  // bisa diklik untuk membuka voucher Memorial (report-table.js).
