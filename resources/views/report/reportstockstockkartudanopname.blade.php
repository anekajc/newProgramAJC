...
{{-- Dulu blok filter halaman ini ada di @section('input'). Di masterreportGudang,
...
<style>
  ...
  /* Samakan baris Grand Total dengan .tb-report .grand-total di report-table.css. */
  #tabel #gtrow td {
...
<div class="tb-report main" style="font-family: 'Segoe UI', sans-serif;">
  <div class="content" style="padding: 20px 24px 0;">
    ...
    <div class="toolbar">
      ...
      <!-- Periode: date1 = tanggal kartu stok, date2 = batas opname (TGLBATASOPNAME) -->
...
<script type="text/javascript">
  ...
  const reportTitle = "LAPORAN KARTU OPNAME";
  ...
  function renderCachedReport() {
    ...
    doShowReport(rowsShown(), reportTitle, reportGroupBy, $("#inputDate1").val(), $("#inputDate2").val());
    doRenameGrandTotal("Total Item : " + rowsShown().length, galign_left);
  }
  ...
  function makeTable(_mode, _callback) {
    ...
    let _date1  = $("#inputDate1").val();   // tanggal kartu stok
    let _date2  = $("#inputDate2").val();   // batas opname (TGLBATASOPNAME)
    ...
    if (_mode !== 'REPORT') {
      doMakeTable(_mode, groupby, data, reportTitle, _date1, _date2, _callback);
      return;
    }
    ...
    doMakeTable(_mode, groupby, data, reportTitle, _date1, _date2, function () {
  ...
  function doExport(fmt) {
    ...
    if (typeof gcart_res !== 'undefined') {
      ...
      a.download = 'LaporanKartuOpname.' + (fmt === 'Excel' ? 'xls' : 'csv');
