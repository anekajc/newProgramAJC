...
{{-- Table styling lives in public/css/report-table.css (loaded via report/newmaster2.blade.php).
     Hutang per Jatuh Tempo: styled .tb-report, dikelompokkan per supplier (namasupp) dengan
     subtotal + grand total (Jumlah/Bayar/Sisa Rp & $). Mode IDR / $ (valas). Kolom (gcart_header)
     interaktif lewat ReportTable (seret/gear/Reset kolom). Mode Valas, Kurs Valas, Perkiraan,
...
@section('header2')
    <div class="tb-report main">
        <div class="content">
            ...
            <div class="toolbar">
                <div>
                    {{-- <div class="page-title">Hutang Per Jatuh Tempo</div>
                    <div class="page-sub">Dicetak oleh: {{ $akses['user'] }} &nbsp;&middot;&nbsp; <span id="printTime"></span>
                    </div> --}}
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="rt-section">
                        ...
                        <div class="rt-grid-1">
                            {{-- Diisi dari *_loadperkiraan (loadPerkiraanDropdown()). Selalu punya nilai
                                 (default akun HT pertama) -- wajib, TIDAK dihitung di badge. --}}
                    ...
                    <div class="rt-section">
                        ...
                        {{-- Supplier difilter per perkiraan; berubah perkiraan mereset pilihan ke '-'. --}}
...
@section('jsreport')
    <script type="text/javascript">
        ...
        var modereport_detail = 11,
            modereport_rekap = 12;
        g_modeReport = modereport_detail;
        var jenisreport = 0,
            DetOrRekap = 0;

        const reportUrl = "{{ url('reportaccountinghutanglhpjt_doReport') }}";

        // Bottom voucher panel endpoints (report-table.js is loaded via masterreport2).
        // Clicking a No Nota calls openVoucher(no, jenisFromNo(no)); the Jenis is parsed
        // from the number (2nd segment), with LPB→BPL handled by jenisFromNo.
        window.ReportTableConfig = {
            kasUrl    : "{{ url('reportaccountinghutanglhpjt_doKasharian') }}",
            invoiceUrl: "{{ url('reportaccountinghutanglhpjt_doInvoice') }}",
            lpbUrl    : "{{ url('reportaccountinghutanglhpjt_doLpb') }}",
            bpUrl     : "{{ url('reportaccountinghutanglhpjt_doBp') }}"
        };

        $(document).ready(function() {
            // #printTime tidak ada di markup saat blok page-title/page-sub dikomentari --
            // dijaga null supaya TIDAK melempar exception yang membatalkan sisa ready()
            // (setReportMode/setOrderBy/loadPerkiraanDropdown di bawah tidak akan jalan
            // kalau baris ini error).
        ...
        /* ── kolom (gcart_header) per mode. Tabel styled DI-RENDER dari sini (Customize Table).
              ...
              & Grand Total. Kolom supplier (namasupp) jadi header grup, tidak ditampilkan lagi. ── */
        ...
        function exportDelimited(fmt) {
            ...
            a.download = 'HutangJatuhTempo_' + (globalDate1 || '') + '_' + (globalDate2 || '') + '.' + ext;
        ...
        /* ── LOAD DATA (sp_ReportSisaHtempo; doReport mengembalikan array biasa) ── */
        function makeTable(_mode) {
            globalDate1 = $('#inputDate1').val();
            globalDate2 = $('#inputDate2').val();
            g_reportTitle = 'REPORT ACCOUNTING HUTANG LAPORAN HUTANG PER JATUH TEMPO';
        ...
        /* ── RENDER: dikelompokkan per supplier (kolom `namasupp`). Subtotal & Grand Total
        ...
        function loadPerkiraanDropdown() {
            let list = [];
            $.ajax({
                url: "{!! url('reportaccountinghutanglhpjt_loadperkiraan') !!}",
                type: "get",
                async: false,
                success: function(res) {
                    list = res || [];
        ...
        function loadSelectSuppAwal() {
            ...
            $.ajax({
                url: "{!! url('reportaccountinghutanglhpjt_loadsuppawal') !!}",
                type: "get",
                async: false,
                data: {
                    perkiraan: perkiraan
        ...
        function loadSelectValas() {
            ...
            $.ajax({
                url: "{!! url('reportaccountinghutanglhpjt_loadvalas') !!}",
                type: "get",
                async: false,
                success: function(res) {
                    dataRefresh = res || [];
        ...
        // Supplier Awal/Akhir & Perkiraan SELALU punya nilai (default akun pertama / '-') --
