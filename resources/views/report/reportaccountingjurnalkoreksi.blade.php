...
{{-- Table styling lives in public/css/report-table.css (loaded via report/newmaster2.blade.php).
     Jurnal Koreksi (BJK): styled .tb-report dengan header dua tingkat (Debet → Perk./Jumlah,
     ...
     pada baris membuka voucher Jurnal Koreksi (CetakKasharian) di panel bawah (report-table.js). --}}
<style>
...
<div class="tb-report main">
  <div class="content">
    ...
    <div class="toolbar">
      <div>
        <div class="page-title">Jurnal Koreksi</div>
...
<script type="text/javascript">
  ...
  const reportUrl = "{{ url('laporanaccountingjurnalkoreksi_doReport') }}";

  // Bottom voucher panel endpoints (report-table.js dimuat oleh masterreport2).
  // No. Bukti di jurnal ini adalah BJK (Bukti Jurnal Koreksi) → openVoucher memakai
  // jenisFromNo() yang memetakan BJK ke CetakKasharian (kasUrl).
  window.ReportTableConfig = {
    kasUrl    : "{{ url('laporanaccountingjurnalkoreksi_doKasharian') }}",
    invoiceUrl: "{{ url('laporanaccountingjurnalkoreksi_doInvoice') }}",
    lpbUrl    : "{{ url('laporanaccountingjurnalkoreksi_doLpb') }}",
    bpUrl     : "{{ url('laporanaccountingjurnalkoreksi_doBp') }}"
  };
  ...
  function exportDelimited(fmt) {
    ...
    a.download = 'JurnalKoreksi_' + (globalDate1 || '') + '_' + (globalDate2 || '') + '.' + ext;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    showToast('📄', 'Data diekspor sebagai ' + fmt);
  }

  /* ── LOAD DATA (Sp_LapJurnal 'BJK', divisi, date1, date2; array biasa) ── */
  function makeTable(_mode) {
    globalDate1 = $('#inputDate1').val();
    globalDate2 = $('#inputDate2').val();
    g_reportTitle = 'REPORT JURNAL KOREKSI';
  ...
  // "Saldo Awal"). Klik dilakukan pada SELURUH baris (lihat voucherRowOpen); Jenis (BJK)
  ...
  // bisa diklik untuk membuka voucher Jurnal Koreksi (report-table.js).
