...
class GudangReturPenyerahanSampleController extends Controller
{
  public function index(Request $req) {
    ...
    $tempOutstanding = DB::connection("SML")->select("
        ...
        WHERE YEAR(A.Tanggal) = :tahun AND MONTH(A.Tanggal) = :bulan and A.IsOtorisasi1 = 0 AND ISNULL(A.pkonsi, 0) = 0
        ORDER BY A.NoBukti", [
        "tahun" => $periode->tahun,
        "bulan" => $periode->bulan
    ]);
    ...
    $tempOutstanding2 = DB::connection("SML")->select("
        ...
        LEFT OUTER JOIN DBCUSTSUPP B ON A.KODECUSTSUPP = B.KODECUSTSUPP
        LEFT OUTER JOIN DBKOTA C ON B.KOTA = C.KodeKota
        WHERE YEAR(A.Tanggal) = :tahun AND MONTH(A.Tanggal) = :bulan and A.IsOtorisasi1 = 1 AND ISNULL(A.pkonsi, 0) = 0
        ORDER BY A.NoBukti", [
        "tahun" => $periode->tahun,
        "bulan" => $periode->bulan
    ]);

    return view('gudang.gudangreturpenyerahansample', [
        "menul0" => $menul0,
        "periode" => $periode,
  ...
  public function loadAll()
{
  ...
  public function updateOtorisasi(Request $req) {
    ...
    $tempX2 =  app('App\Http\Controllers\GlobalController')->LoggingData( 'oto','RSP',$req->nobukti,'',0,'DBRSERAHSAMPLE');
    return $res;
}
  public function updateBatalOtorisasi(Request $req) {
    $res = DB::connection('SML')->update(
     ...
     $tempX2 =  app('App\Http\Controllers\GlobalController')->LoggingData( 'btloto','RSP',$req->nobukti,$req->pket,0,'DBRSERAHSAMPLE');
    return $res;
}
...
public function SerahSample (Request $req)
{
...
//         foreach ($exclude as $item) {
//             if (!empty($item['nobukti']) && !empty($item['urut'])) {
//                 $excludeClauses[] = "(A.NOBUKTI != ? OR A.Urut != ?)";
//                 $bindings[] = $item['nobukti'];
//                 $bindings[] = $item['urut'];
//             }
//         }
...
// }
  ...
  public function getNoBukti (Request $req) {
    ...
    $username = \Auth::user()->username;
    $periode = app('App\Http\Controllers\GlobalController')->getPeriode();
    $inisial = DB::connection("SML")->select('select RSS from DBNOMOR');
...
//     WHERE a.isaktif = 1
//     AND (a.Kodebrg LIKE ? OR a.NamaBrg LIKE ? OR a.partNumber LIKE ?)
//     ORDER BY a.Kodebrg", [
//     $search,
//     $search,
//     $search
...
//  return ["asd" => $nobukti] ;
    ...
    if ($choice =='D'){
      $tempX2 =  app('App\Http\Controllers\GlobalController')->LoggingData( $req->choice,'RSP',$nobukti,'',$xurut,'DBRSERAHSAMPLEdet');
      }
  ...
  public function spDelete(Request $req)
{
    ...
    $username = \Auth::user()->username;
     ...
     $tempX2 =  app('App\Http\Controllers\GlobalController')->LoggingData( $req->choice,'RSP',$nobukti,'',$req->urut,'DBRSERAHSAMPLEdet');
