...
class GiroDiterimaController extends Controller
...
",[ "tahun" =>$periode->tahun , "bulan" => $periode->bulan ]);
    ...
    return view('accounting.giroditerima' , [
      "menul0" => $menul0,
      "periode" => $periode,
  ...
  public function loadAll (Request $req) {
    ...
    // tglawal/tglakhir sebagai date-range bebas -- kalau tidak dikirim (mis. pemanggilan
    // lama), jatuh ke default bulan/tahun periode yang sedang aktif supaya perilaku awal
    // tidak berubah.
  ...
  public function listPencairanGiro (Request $req) {
    $listGiro = DB::connection("SML")->select("
    Select Bank, Nogiro, TglGiro, Case when kodevls='IDR' then DebetRp else Debet end jumlah,
KodeVls, Kurs, Keterangan
from dbGiro
where Tipe='PT' and TglCair is null
...
" , ["nobukti" => $req->nobukti]);
 ...
 and Perkiraan in (select Perkiraan from dbPostHutPiut where Kode='GTR')
and  Perkiraan in (select Perkiraan from dbAksesPerkiraan where UserID= :username )
...
WHERE KODESUPP= :custsupp and IDuser= :username", ["custsupp" => $req->custsupp , "username1" =>  \Auth::user()->username , "username" =>  \Auth::user()->username]);
    ...
    // DB::connection('SML')->statement('exec sp_TransaksiKasBankDPH ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?', [$values]);
      ...
      // foreach ($tempData as $d)  {
        ...
        $values = [
          ...
          $req->transaksi ,
          'P',
          $req->transaksi == 'BGT' ? $req->kodecustsupp : '' ,
          $req->transaksi == 'BGT' ? $req->kodecustsupp : '' , //
          $req->urut ,
          '',
          '',
  ...
  DB::connection('SML')->statement('exec sp_TransaksiGiroTerima ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?', $values);
  ...
  foreach ($tempData as $d) {
    ...
    DB::connection('SML')->update("insert into dbgiro (NoGiro, Bank , TglGiro, BuktiBuka , UrutBuktiBuka, TglBuka, KodeVls , Kurs , DebetRp , KreditRp , Debet , Kredit , Keterangan , Kas , Tipe)
          ...
          " , [
            ...
            "valas" => $d['valas'],
            "kurs" => $d['kurs'],
            "debetrp" => (int)$d['nilaigiro'] * (int)$d['kurs'],
            "kreditrp" => 0,
            "debet" => (int)$d['nilaigiro'],
            "kredit" => 0,
    ...
    if ($req->choice == "I") {
      DB::connection('SML')->update("insert into dbgiro (NoGiro, Bank , TglGiro, BuktiBuka , UrutBuktiBuka, TglBuka, KodeVls , Kurs , DebetRp , KreditRp , Debet , Kredit , Keterangan , Kas , Tipe)
            ...
            " , [
              ...
              "valas" => $req->valas,
              "kurs" => $req->kurs,
              "debetrp" => (int)$req->nilaigiro * (int)$req->kurs,
              "kreditrp" => 0,
              "debet" => (int)$req->nilaigiro,
              "kredit" => 0,
              "keteranganhd" => $req->kepadaterima ? $req->kepadaterima : '',
              "lawan" => $req->kodeperkiraan,
              "tipe" => 'PT'
