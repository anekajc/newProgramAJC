...
class LaporanMarketingHistoryOutSOController extends Controller
{
    use AksesTrait;
    use GlobalTrait;
    ...
    {
        $akses = $this->cekAkses("laporanmarketinghistoryoutso");
        if ($akses['userLoggedOut']) {
            return redirect('/');
        }

        if ($akses['akses']->Access) {
            return view('report.reportmarketinghistoryoutso', [
                "akses" => $akses
            ]);
        } else {
    ...
    public function doReport(Request $req)
    {
        ...
        $values  = [$tgl1, $tgl2, $KodeCust, $lokasi, $IDUser, $TipeTrans, $isout];

        $res = DB::connection('SML')->select(
            'exec SP_REPORTHISSOOUTweb ?,?,?,?,?,?,?',
            $values
        );
    ...
    public function doFilter(Request $req)
    {
        ...
        $listData = DB::connection('MGL')->select('select ' . $kolom . ' from Vwreporthissoout where tanggal between :tgl1 and :tgl2 group by ' . $kolom, ['tgl1' => $req->date1, 'tgl2' => $req->date2]);
        return $listData;
    }
