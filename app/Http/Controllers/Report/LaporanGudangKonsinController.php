...
class LaporanGudangKonsinController extends Controller {
  use AksesTrait;
  use GlobalTrait;

  public function index() {
    $akses = $this->cekAkses("laporangudangkonsin");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('reportgudangkonsin' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doReport(Request $req) {
    ...
    $res = DB::connection('MGL')->select("SELECT * from VWREPORTSERAHSAMPLE where tanggal 
between :date1 and :date2  and cast (KodeSls as varchar(2)) like '' and NObukti LIke '%SPK%' order by tanggal,nobukti",
