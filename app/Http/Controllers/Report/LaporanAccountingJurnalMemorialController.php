...
class LaporanAccountingJurnalMemorialController extends Controller {
  use AksesTrait;
  use GlobalTrait;
  ...
  public function index() {
    $akses = $this->cekAkses("laporanaccountingjurnalmemorial");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportaccountingjurnalmemorial' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doReport(Request $req) {
    ...
    $res = DB::connection('MGL')->select("exec Sp_LapJurnal 'BMM',:divisi,:date1,:date2",
