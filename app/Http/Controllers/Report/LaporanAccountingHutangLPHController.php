...
class LaporanAccountingHutangLPHController extends Controller {
  use AksesTrait;
  use GlobalTrait;
  use ReportVoucherTrait;   // doKasharian / doInvoice / doLpb / doBp (bottom voucher panel)

  public function index() {
    $akses = $this->cekAkses("reportaccountinghutanglph");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportaccountinghutanglph' , [
        "akses" => $akses
      ]);
    } else {
  ...
  // SP sama dengan report Hutang Kartu/Umur; kodesupp awal = akhir = supplier yang diklik.
