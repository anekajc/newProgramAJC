...
class LaporanAccountingLabaRugiTahunanController extends Controller {
  use AksesTrait;
  use GlobalTrait;

  public function index() {
    $akses = $this->cekAkses("laporanaccountinglabarugitahunan");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportaccountinglabarugitahunan' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doReport(Request $req) {
    ...
    $res = DB::connection('MGL')->select("exec sp_ReportLabaRugiTH :inputBulan,:inputTahun,:divisi,0,:totalA,:totalB,:totalC ",
