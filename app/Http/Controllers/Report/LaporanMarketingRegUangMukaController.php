...
class LaporanMarketingRegUangMukaController extends Controller {
  use AksesTrait;
  use GlobalTrait;

  public function index() {
    $akses = $this->cekAkses("laporanmarketingreguangmuka");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportmarketingreguangmuka' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doReport(Request $req) {
    ...
    $values  = [$SReport, $Ordr, $tgl1, $tgl2, $isiList, $NeedOto, $IdUser, $Tipe, $BJ];

    $res = DB::connection('SML')->select('exec Sp_reportUMDet ?,?,?,?,?,?,?,?,?',
      $values);
  ...
  public function doFilter(Request $req) {
    ...
    $listData = DB::connection('MGL')->select('select ' . $kolom . ' from Vwreportumdet where tanggal between :tgl1 and :tgl2 group by ' . $kolom , ['tgl1' => $req->date1, 'tgl2' => $req->date2]);
    return $listData;
  }
