...
class LaporanMarketingOutSPBHrgSoController extends Controller {
  use AksesTrait;
  use GlobalTrait;

  public function index() {
    $akses = $this->cekAkses("laporanmarketingoutspbhrgso");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportmarketingoutspbhrgso' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doFilter(Request $req) {
    $kolom = ($req->get('inputOrd') == "X") ? 'NoBukti, Tanggal' : 'KodeBrg, NamaBrg';
  ...
  public function doReportFilter(Request $req) {
    $kolom = ($req->get('inputOrd') == "X") ? 'NoBukti' : 'KodeBrg';
    $res = [];
