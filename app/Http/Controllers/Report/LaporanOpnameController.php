...
class LaporanOpnameController extends Controller {
  ...
  public function index() {
    ...
    if ($akses['akses']->Access) {
      return view('report.reportopname' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doReport(Request $req) {
    ...
    $res = DB::connection('MGL')->select('exec Sp_ReportOpnameBarang ?,?,?,?,?,?,?,?',
      $values);
  ...
  public function doFilter(Request $req) {
    ...
    $listData = DB::connection('MGL')->select('select ' . $kolom . ' from VwreportOPnamebarang where tanggal between :tgl1 and :tgl2 group by ' . $kolom , ['tgl1' => $req->date1, 'tgl2' => $req->date2]);
    return $listData;
  }
  ...
  public function doReportFilter(Request $req) {
    ...
    for ($i=0; $i < count($req->listdata); $i++) {
      $row = DB::connection('MGL')->select('select * from VwreportOPnamebarang where ' . $kolom . ' = :list' , ['list' => $req->listdata[$i]]);
