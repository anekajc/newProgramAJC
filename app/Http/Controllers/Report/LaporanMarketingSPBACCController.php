...
class LaporanMarketingSPBACCController extends Controller {
  use AksesTrait;
  use GlobalTrait;

  public function index() {
    $akses = $this->cekAkses("laporanmarketingspbacc");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportlaporanmarketingspbacc' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doReport(Request $req) {
    ...
    $res = DB::connection('SML')->select('exec Sp_ReportSPBACCDet ?,?,?,?,?,?,?,?',
      $values);
