...
class LaporanAccountingJurnalComputerController extends Controller {
  use AksesTrait;
  use GlobalTrait;

  public function index() {
    $akses = $this->cekAkses("laporanaccountingjurnalcomputer");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportaccountingjurnalcomputer' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doReport(Request $req) {
    ...
    $res = DB::connection('MGL')->select("exec Sp_LapJurnal 'CM',:divisi,:date1,:date2",
