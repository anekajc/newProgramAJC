...
class LaporanAccountingJurnalPenerimaanKasController extends Controller {
  use AksesTrait;
  use GlobalTrait;
  // Bukti Kas voucher panel (doKasharian/doInvoice/doLpb/doBp). The controller's own
  // doFilter/doReportFilter/loadDivisi take precedence over the trait's same-named methods.
  use ReportVoucherTrait;

  public function index() {
    $akses = $this->cekAkses("laporanaccountingjurnalpenerimaankas");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportaccountingjurnalpenerimaankas' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doReport(Request $req) {
    ...
    $res = DB::connection('MGL')->select("exec Sp_LapJurnal 'BKM',:divisi,:date1,:date2",
  ...
  public function loadPerkiraan1 () {
    $listData = DB::connection('SML')->select("select Perkiraan,Keterangan from dbperkiraan where Kelompok = '4' and Tipe = '1'");
    return $listData;
  }
