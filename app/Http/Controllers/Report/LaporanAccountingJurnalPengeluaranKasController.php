...
class LaporanAccountingJurnalPengeluaranKasController extends Controller {
  use AksesTrait;
  use GlobalTrait;
  ...
  public function index() {
    $akses = $this->cekAkses("laporanaccountingjurnalpengeluarankas");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportaccountingjurnalpengeluarankas' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doReport(Request $req) {
    ...
    $res = DB::connection('SML')->select("exec Sp_LapJurnal 'BKK',:divisi,:date1,:date2",
