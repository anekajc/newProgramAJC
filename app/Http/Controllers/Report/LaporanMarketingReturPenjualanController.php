...
class LaporanMarketingReturPenjualanController extends Controller {
  use AksesTrait;
  use GlobalTrait;

  public function index() {
    $akses = $this->cekAkses("laporanmarketingreturpenjualan");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportmarketingreturpenjualan' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doReport(Request $req) {
    ...
    $res = DB::connection('SML')->select('exec Sp_ReportRInvoicePenjualanDet ?,?,?,?,?,?',
      $values);
