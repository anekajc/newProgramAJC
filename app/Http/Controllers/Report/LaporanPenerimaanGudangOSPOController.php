...
class LaporanPenerimaanGudangOSPOController extends Controller {
  ...
  public function doFilter(Request $req) {
    ...
    $listData = DB::connection('MGL')->select('select ' . $kolom . ' from VwreportoutStandingPOdet where tanggal between :tgl1 and :tgl2 group by ' . $kolom , ['tgl1' => $req->date1, 'tgl2' => $req->date2]);
    return $listData;
  }
  ...
  public function doReportFilter(Request $req) {
    ...
    for ($i=0; $i < count($req->listdata); $i++) {
      $row = DB::connection('MGL')->select('select * from VwreportoutStandingPOdet ' . $kolom . ' = :list' , ['list' => $req->listdata[$i]]);
