...
class LaporanMarketingUangMukaOutController extends Controller {
  use AksesTrait;
  use GlobalTrait;

  public function index() {
    $akses = $this->cekAkses("laporanmarketinguangmukaout");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportmarketinguangmukaout' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doReport(Request $req) {
    ...
    $res = DB::connection('SML')->select('exec Sp_REPORTOUTUMJUALDET ?,?,?,?,?,?,?,?',
      $values);
  ...
  public function loadSales () {
    $listData = DB::connection('SML')->select('select KeyNIK KodeSls,NIK,Nama namaSls from dbKaryawan where  Aktif=1  and IsSales=1');
    return $listData;
  }
