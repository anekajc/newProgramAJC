...
class LaporanMarketingInvoiceController extends Controller {
  use AksesTrait;
  use GlobalTrait;

  public function index() {
    $akses = $this->cekAkses("laporanmarketinginvoice");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportmarketinginvoice' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function loadGroup2 () {
    $listData = DB::connection('SML')->select('select KODEGROUPCUSTSUPP,NamaGROUPCUSTSUPP from DBGROUPCUSTSUPP');
    return $listData;
  }
