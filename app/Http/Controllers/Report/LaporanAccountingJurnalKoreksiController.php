...
class LaporanAccountingJurnalKoreksiController extends Controller {
  use AksesTrait;
  use GlobalTrait;
  ...
  public function index() {
    $akses = $this->cekAkses("laporanaccountingjurnalkoreksi");
    if ($akses['userLoggedOut']) { return redirect('/'); }

    if ($akses['akses']->Access) {
      return view('report.reportaccountingjurnalkoreksi' , [
        "akses" => $akses
      ]);
    } else {
  ...
  public function doReport(Request $req) {
    ...
    $res = DB::connection('SML')->select("exec Sp_LapJurnal 'BJK',:divisi,:date1,:date2",
