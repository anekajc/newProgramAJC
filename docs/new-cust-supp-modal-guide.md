# New Design — Entity Picker Modal Guide (Cust/Supp and friends)
...
Despite the filename, this applies to **every entity** the picker is used for — Customer,
Supplier, Lokasi/Kebun, PIC, Kategori, Sub-Kategori, Merk, Perkiraan, Divisi, Sales, Group.
Cust/Supp is just the most common case.

**What this produces:** a picker where the whole row is clickable — **no Actions column, no green
Select button** — plus a modern modal shell and skinned DataTables controls.
...
`loadSelectXxx()` (one per entity: AJAX → build header + rows → init DataTables), and
...
> skinned — either rename the id, or add that id to the selector group in
...
**Live include counts** (excluding the dated backup folders — see the all-guide):

| Modal blade | Included by | Picker style |
|---|---|---|
| `modalMarketingSO.blade.php` | 19 pages | **gated** — needs `window.g_pickerV2 = true` |
| `modalAccountingJurnal.blade.php` | 8 pages | **ungated** — always click-row |
| `modalMarketingHistorySO.blade.php` | 2 pages | **ungated** — always click-row |
| `modalMarketingInvoice.blade.php` | 1 page | **gated** |
| `modalMarketingRegUangMuka.blade.php` | 1 page | **gated** |
| `modalStock.blade.php` | **0 pages** | dead code — don't bother |

### Gated vs ungated

**Gated** — everything sits behind `window.g_pickerV2`. With the flag falsy the markup is
...
**Ungated** — the Actions column and its button are deleted outright, every row is clickable, and
...
> highlight live inside the `#formSelect.rt-picker-v2 …` selectors — without the class you get
...
page** → stop and ask (see the all-guide).
...
> copying — if the old markup varies, either add a parameter for it or go ungated.

### Step 2 — refactor each `loadSelectXxx()`
...
Keep `idPart` and the value passed to `buttonPilih` **exactly** as they were — they map to the
page's hidden inputs. Drop the unused `let temp = "";`.

### Step 3 — the class toggle in `buttonSelect()`
...
### Step 4 — (gated only) opt the page in

```js
window.g_pickerV2 = true;
```

Leave the `DataTable({...})` init alone — its search box, info line and pagination are skinned by
CSS, not replaced.

---

## 5. Hard rule — never include two picker modals on one page
...
- **Duplicate DOM id** — two `<div id="formSelect">`. `$("#formSelect")` only matches the first,
...
- **Silently overwritten functions** — both declare `buttonSelect`, `buttonPilih`,
  `loadSelectCustomer`, … As plain `<script>` declarations the last include wins, so a loader can
  fire the *other* modal's AJAX route and populate a table nobody can see.

This already happened and was fixed by deleting one include — the note is still in
...
the modal it already includes** — don't include the second one. Adding a loader to a shared modal
is itself a shared-file edit → ask first.
...
| Symptom | Cause / fix |
|---|---|
| Rows clickable but no pointer/hover | `rt-picker-v2` class not applied — check `buttonSelect()` |
| Skin doesn't apply | modal id isn't `formSelect`, or CSS not loaded on this layout |
| Column order changed after conversion | old markup had Actions **first** (e.g. `modalAccountingJurnal`) — see Step 1 |
| Button restyled unexpectedly | old markup used `btn-primary`/`+`, not `btn-success`/"Select" |
| Wrong field gets filled | `idPart` changed — it maps to a specific hidden input in `buttonPilih()` |
| Picker opens but table is empty / wrong data | two picker modals included → duplicate `#formSelect` (§5) |
| Modal won't close | open/close going through different Bootstrap versions — see the all-guide |
...
- [ ] Decided gated vs ungated — and if ungated, confirmed the blast radius with the user
